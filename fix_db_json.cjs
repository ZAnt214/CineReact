const fs = require('fs');
if (fs.existsSync('db_cine_react.json')) {
  let db = JSON.parse(fs.readFileSync('db_cine_react.json', 'utf8'));
  const novos = [
    {
      id: 'marvel',
  ...
  novos.forEach(n => {
    if (!db.obras.find(o => o.id === n.id)) {
      db.obras.push(n);
    }
  });
  fs.writeFileSync('db_cine_react.json', JSON.stringify(db, null, 2));
}
