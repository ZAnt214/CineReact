    ...
    >
      ...
      <div className="relative h-64 md:h-80 w-full rounded-2xl overflow-hidden mb-12 shadow-2xl border border-zinc-800">
        ...
        <div className="absolute bottom-0 left-0 p-8 w-full">
          <div className="flex flex-col gap-2">
            <span className="text-stone-300 font-black tracking-widest text-xs uppercase bg-stone-400/10 px-3 py-1 rounded-full w-max border border-stone-400/20">
              {obra.tipo}
            </span>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-8 border-b border-zinc-800 pb-4">
        <button 
          ...
          className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${filter === 'relevantes' ? 'bg-stone-300 text-black shadow-lg shadow-stone-300/20' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800 hover:text-white border border-zinc-800'}`}
        >
          Mais Relevantes
        </button>
        <button 
          onClick={() => setFilter('recentes')}
          className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${filter === 'recentes' ? 'bg-stone-300 text-black shadow-lg shadow-stone-300/20' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800 hover:text-white border border-zinc-800'}`}
        >
          Mais Recentes
        </button>
        <button 
          onClick={() => setFilter('antigos')}
          className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${filter === 'antigos' ? 'bg-stone-300 text-black shadow-lg shadow-stone-300/20' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800 hover:text-white border border-zinc-800'}`}
        >
          Mais Antigos
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {filteredReacts.map(react => (
          <motion.div
            ...
            className="bg-zinc-900/30 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg border border-zinc-855 hover:border-stone-400/40 transition-colors cursor-pointer group/card flex flex-col select-none"
          >
            <div className="relative aspect-video w-full overflow-hidden bg-zinc-950">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/card:opacity-100 transition-opacity flex items-center justify-center">
                <div className="w-12 h-12 rounded-full bg-stone-400/90 flex items-center justify-center shadow-lg shadow-stone-400/30">
            ...
            <div className="p-4 flex flex-col flex-1">
              <h3 className="text-sm font-bold text-white line-clamp-2 leading-snug group-hover/card:text-stone-300 transition-colors mb-2">
                {react.titulo}
              </h3>
